  ...
  return (
    <div className="mc">
      ...
      <ul className="mc__list">
        {live.map((m) => <MatchRow key={m.id} m={m} live name={name} />)}
      </ul>
      <h2 className="mc__h">Upcoming</h2>
      <ul className="mc__list">
        {upcoming.map((m) => <MatchRow key={m.id} m={m} name={name} />)}
      </ul>
    </div>
  )
}

function MatchRow({ m, live, name }: { m: Match; live?: boolean; name: (id: number) => string }) {
  return (
    <li className={`mc__row ${live ? 'is-live' : ''}`}>
      <span className="mc__teams">{name(m.homeId)} v {name(m.awayId)}</span>
